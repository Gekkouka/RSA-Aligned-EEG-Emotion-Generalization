...
class SEEDFeatureDataset(object):
    ...
    def _sampleIO(self, root_path, info, channel_index, feature):
        ...
        trial_ids = sorted(
            [
                int(re.findall(r"de_LDS(\d+)", key)[0])
                for key in eeg.keys()
        ...
        for trial_id in trial_ids:
            ...
            # 将group拼接
            win_id = np.arange(num_trial, dtype=np.int16).reshape(-1, 1)
            group_trial = np.hstack((group_session_id, group_subject_id, group_trial_id, win_id))
