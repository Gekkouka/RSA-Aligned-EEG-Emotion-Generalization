...
class DEAPDataset(SignalDataset):
    ...
    def _process_one_subject(
        ...
        # add window index (1..n_slices) per trial
        n_trials = labels.shape[0]
        n_slices = stimulus_group.shape[0] // n_trials
        window_idx = np.tile(np.arange(1, n_slices + 1), n_trials)
        stimulus_group = np.column_stack((stimulus_group, window_idx))
        # extract features
        ...
        stimulus_group = np.column_stack((np.full_like(stimulus_group[:, 0], subject_id), stimulus_group))
